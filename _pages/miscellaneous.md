---
layout: archive # should be wide
title: "Miscellaneous"
permalink: /miscellaneous/
author_profile: true
redirect_from: 
  - /misc/
---

<!-- 放一些杂项 -->

---

# 💾Links

- USTC: [en.ustc.edu.cn](https://en.ustc.edu.cn){:target="_blank"} 
- SGY, USTC: [en.scgy.ustc.edu.cn](https://en.scgy.ustc.edu.cn){:target="_blank"} 
- Math, USTC: [math.ustc.edu.cn](http://math.ustc.edu.cn/ENGLISH/list.htm){:target="_blank"} 
- LibGen: [libgen.gs](https://libgen.gs){:target="_blank"} 
- iCourse Club, USTC: [icourse.club](https://icourse.club){:target="_blank"} 
- Intel OneAPI Toolkits: [OneAPI](https://www.intel.com/content/www/us/en/developer/tools/oneapi/toolkits.html){:target="_blank"} 
- S.-T. Yau College Student Mathematics Contest: [Yau-Contest](http://yau-contest.com/en){:target="_blank"} 
- Warming Math Magazine (USTC Math Department): [Warming](http://staff.ustc.edu.cn/~mathsu01/pu/waming.html){:target="_blank"} 
- AcademicPages (template for this website): [academicpages](https://github.com/academicpages/){:target="_blank"} 
- VSCode portable: [VSCode portable](https://code.visualstudio.com/docs/editor/portable){:target="_blank"} 
- [How to use `ifort` + `IntelMPI`/`coarray` on Windows or Linux clusters](https://blog.csdn.net/PilotJohnWu/article/details/121064266){:target="_blank"} 


# 📷My Homecity

<center>
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/from-hill-hui.jpg" 
        width = "24%">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/nanchang-st.jpg" 
        width = "24%">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/bridge-on-river.jpg" 
        width = "24%">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/bird-on-taihu.jpg" 
        width = "24%">
    <br />
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/lake-li-1.jpg" 
        width = "24%">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/lake-li-2.jpg" 
        width = "24%">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/turtle-head-peninsula.jpg" 
        width = "24%">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/yangtze-river.jpg" 
        width = "24%">
    <br />
    <div style = "
        color: orange;
        display: inline-block;
        color: #999;
        padding: 2px;">
        my homecity Wuxi, the pearl of Taihu Lake, featuring both modernity and traditional Chinese culture in the Yangtze Delta (photos are taken by myself) 
    </div>
    <p> </p>
</center>

---

# Explore the world

<center>
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-6.jpg" 
        width = "30%"
        title = "03/24/2024: 埃及白沙漠国家公园 (White Desert National Park, Egypt)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-11.jpg" 
        width = "30%"
        title = "03/25/2024: 埃及白沙漠国家公园 (White Desert National Park, Egypt)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-19.jpg" 
        width = "30%"
        title = "07/07/2024: 新疆省伊犁州特克斯县喀拉峻大草原 (Kalajun Prairies, Tekes County, Ili, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-3.jpg" 
        width = "30%"
        title = "07/10/2024: 新疆省库车市独库公路 (National Highway G217, Kucha, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-8.jpg" 
        width = "30%"
        title = "08/14/2024: 青海省茫崖市黑独山 (Heidu Mountain, Mangnai, Qinghai Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-16.jpg" 
        width = "30%"
        title = "08/15/2024: 青海省格尔木市G315国道 (National Highway G315, Golmud, Qinghai Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-20.jpg" 
        width = "30%"
        title = "01/09/2024: 新疆省喀什地区塔什库尔干县帕米尔高原喀喇昆仑公路 (Karakoram Highway on the Pamir Plateau, Tashkurgan County, Kashgar, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-23.jpg" 
        width = "30%"
        title = "03/26/2025: 埃及锡瓦撒哈拉沙漠 (Sahara Desert, Siwa, Egypt)">
    <br />
    <div style = "
        color: orange;
        display: inline-block;
        color: #999;
        padding: 2px;">
        deserts, prairies and plateaux
    </div>
    <p> </p>
</center>

<center>
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-13.jpg" 
        width = "30%"
        title="10/09/2023: 美国新罕布什尔州白山国家公园 (White Mountains, NH, USA)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-1.jpg" 
        width = "30%"
        title="12/30/2023: 安徽省潜山市天柱山 (Tianzhu Mountain, Qianshan, Anhui Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-2.jpg" 
        width = "30%"
        title = "01/09/2024: 云南省丽江市玉龙县文海路 (Wenhai Rd., Yulong County, Lijiang, Yunnan Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-18.jpg" 
        width = "30%"
        title = "07/05/2024: 新疆省伊犁州伊昭公路S237 (Provincial Highway S237, Qapqal Xibe County, Ili, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-15.jpg" 
        width = "30%"
        title = "08/12/2024: 青海省海北州祁连县G213国道 (National Highway G213, Qilian County, Haibei, Qinghai Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-5.jpg" 
        width = "30%"
        title = "01/07/2024: 新疆省克州阿克陶县奥义塔克镇红峡谷 (Red Valley, Oytak Town, Akto County, Kizilsu, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-10.jpg" 
        width = "30%"
        title = "01/09/2024: 新疆省喀什地区塔什库尔干县慕士塔格四号冰川 (Muztagh Ata No. 4 Glacier, Tashkurgan County, Kashgar, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-21.jpg" 
        width = "30%"
        title = "01/11/2024: 新疆省克州乌恰县天山昆仑山交汇 (Tianshan & Kunlun Mountains Interchange Landform, Ulugqat County, Kizilsu, Xinjiang Prov., China)">
    <br />
    <div style = "
        color: orange;
        display: inline-block;
        color: #999;
        padding: 2px;">
        mountains, valleys and glaciers
    </div>
    <p> </p>
</center>

<center>
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-4.jpeg" 
        width = "30%"
        title = "01/31/2023: 江苏省无锡市太湖鼋头渚 (Turtle Head Island, Taihu Lake, Wuxi, Jiangsu Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-17.jpg" 
        width = "30%"
        title = "01/14/2024: 云南省大理市洱海 (Lake Erhai, Dali, Yunnan Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-12.jpg" 
        width = "30%"
        title = "03/30/2024: 埃及赫尔格达红海 (the Red Sea, Hurghada, Egypt)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-14.jpg" 
        width = "30%"
        title = "06/14/2024: 浙江省杭州市淳安县千岛湖 (Qiandao Lake, Chun'an County, Hangzhou, Zhejiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-9.jpg" 
        width = "30%"
        title = "07/05/2024: 新疆省博乐市赛里木湖 (Sayram Lake, Bole, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-7.jpg" 
        width = "30%"
        title = "01/07/2024: 新疆省克州阿克陶县白沙湖 (Baisha Lake, Akto County, Kizilsu, Xinjiang Prov., China)">
    <img style = "
        border-radius: 0.3125em;
        box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
        src = "../files/pictures/me-22.jpg" 
        width = "30%"
        title = "03/24/2025: 埃及马特鲁地中海 (the Mediterranean Sea, Matruh, Egypt)">
    <br />
    <div style = "
        color: orange;
        display: inline-block;
        color: #999;
        padding: 2px;">
        lakes and seas
    </div>
    <p> </p>
</center>

<a href="https://my.flightradar24.com/pilotjohnwu"><img src="https://banners-my.flightradar24.com/pilotjohnwu.png" alt="My Flightdiary.net profile" /></a>
